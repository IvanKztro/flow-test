<script>
    ...
    const {section} = $props()
    const getWidth= ()=> section.settings.width =='lg' ? 'mx-auto max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mt-24' : 'md:mx-auto md:max-w-2xl'
    const getImageSize = () => section.settings.isImageSquare ? 'aspect-square' : 'h-auto'
</script>

<!-- <div class="md:mx-auto md:max-w-2xl"> -->
<div class="{getWidth()}">
	<div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
		{#each Array(4).fill(0) as _, colIndex}
			<div class="space-y-2">
				{#each images.slice(colIndex * 3, colIndex * 3 + 3) as image}
					<img class="w-full object-cover {getImageSize()}  " src={image.src} alt={image.alt} />
				{/each}
			</div>
		{/each}
	</div>
...
<!-- 
<div class="mx-auto max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mt-24">
	<div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
		{#each Array(4).fill(0) as _, colIndex}
			<div class="space-y-2">
				{#each images.slice(colIndex * 3, colIndex * 3 + 3) as image}
					<img class="h-auto w-full object-cover" src={image.src} alt={image.alt} />
				{/each}
			</div>
		{/each}
	</div>
