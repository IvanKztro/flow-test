...
{:else if firekitUser.user}
  <DropdownMenu.Root>
    <DropdownMenu.Trigger>
      <Avatar.Root>
        <Avatar.Image src={firekitUser.user?.photoURL} alt="Avatar" />
        <Avatar.Fallback>
          {getInitials(firekitUser.user?.displayName)}
        </Avatar.Fallback>
      </Avatar.Root>
    </DropdownMenu.Trigger>
        ...
        <DropdownMenu.GroupHeading>
          <div class="flex items-center gap-3">
            ...
            <div class="grow">
              ...
              >
                {firekitUser.user?.displayName}
              </span>
              <p class="text-xs text-foreground-500">
                {firekitUser.user?.email}
              </p>
            </div>
          </div>
        </DropdownMenu.GroupHeading>
